...
def removeOutliers(ad):
    ...
    s = {}
    for sID in ad.obs.batch.sort_values().unique():
        print(sID)
        s[sID] = ad[ad.obs['batch']==sID]
...
newAD = sm.pp.rescale(newAD,imageid='batch', method='by_image')
...
newAD.obs.groupby('batch').phenotype.value_counts()
